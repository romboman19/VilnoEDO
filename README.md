...
JSON manifest можна отримати через token-bound endpoint:

```text
GET /api/ua-kep/evidence/:evidencePackageId/manifest.json?recipientId=...&recipientToken=...&envelopeId=...
```

Endpoint повертає `404`, якщо `recipientId`, `recipientToken`, `envelopeId` і `evidencePackageId` не належать одному підписанню.
...
- ZIP export bundle для evidence package;
- реальний validation engine поверх IIT/КНЕДП trust material;
- UI для перегляду UA KEP artifacts, validation status і evidence package;
- інтеграція VilnoCheck-SignService як окремого signing/validation сервісу;
- cleanup upstream paid/license gates, які не відповідають цілі вільного self-hosted ЕДО;
...
VilnoEDO стартував як fork Documenso. Ми зберігаємо сильні сторони базового ядра документообігу, але розвиваємо окремий український продукт із власною логікою, термінологією, signing flow і evidence pipeline.
