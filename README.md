...
VilnoEDO працює в **UA-only** режимі: єдиний доступний рівень підпису — український
КЕП/УЕП/електронна печатка (`UA_KEP`). Створення конвертів з non-UA рівнями
(SES/AES/QES) API відхиляє, а у формах створення документа їх не пропонують.

> **`.p12` і TLS.** Інстанс-сертифікат `.p12` Documenso — це legacy серверна печатка
> upstream-флоу; у VilnoEDO вона **не використовується** і не є юридичним механізмом
> підпису (його відсутність ні на що не впливає, на startup не перевіряється). TLS-сертифікат
> (Let's Encrypt) потрібен лише для HTTPS-доступу до вебзастосунку і **не є** сертифікатом
> електронного підпису документа.
