...
- приймальна верифікація на бекенді має плагіновий шов: структурна fail-closed перевірка (парсинг CMS, messageDigest, чинність сертифіката, видавець з реєстру) завжди є операційною підлогою прийняття документа, а повна криптографічна верифікація (ДСТУ-4145, ланцюг довіри, trust material) делегується зовнішньому авторитетному validation-сервісу через `POST /api/verify` за наявним контрактом; сервіс вмикається змінною `NEXT_PRIVATE_UA_KEP_VERIFY_SERVICE_URL`, а поки не заданий — шов дормантний і прийняття тримає структурна перевірка. Legal class (KEP/UEP_QC/AdES) фіксується у validation report.
...
### Приймальна верифікація — на бекенді

- **Структурна fail-closed перевірка** (парсинг CMS SignedData, звірка messageDigest, чинність сертифіката на час підпису, видавець з реєстру КНЕДП) виконується в процесі застосунку і є операційною підлогою прийняття документа.
- **Повна криптографічна верифікація** (ДСТУ-4145, ланцюг довіри, trust material) реалізована як плагіновий рушій за чистим інтерфейсом. Поточний рушій — виклик зовнішнього авторитетного validation-сервісу, що повертає відтворюваний підписаний звіт. Рушій дормантний, доки не заданий `NEXT_PRIVATE_UA_KEP_VERIFY_SERVICE_URL`.

### Збереження доказів

Незмінні оригінальні байти документа, detached CAdES artifacts, validation reports, trust material snapshot, audit log і canonical evidence manifest з SHA-256.

Такий поділ дозволяє окремо розвивати юридично і криптографічно чутливу частину, не змішуючи її з основним продуктом документообігу, і не робить жоден зовнішній сервіс єдиною точкою прийняття документа.
...
- визначення й інтеграція авторитетного validation-сервісу для повної ДСТУ-4145 верифікації (офіційний ЦЗО-звіт або кваліфікований провайдер) і e2e-тест на реальних ключах КНЕДП;
