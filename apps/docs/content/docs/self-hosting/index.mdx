...
Self-hosted VilnoEDO runs without a Documenso license key or license server. Core document workflows and unlocked self-hosted limits are available by default.

If an enterprise-grade capability is needed later, implement and document it as a VilnoEDO feature rather than relying on Documenso licensing.
