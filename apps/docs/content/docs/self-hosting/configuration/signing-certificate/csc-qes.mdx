...
<Callout type="warn">
  CSC mode no longer requires a Documenso license key in VilnoEDO, but it still requires a compatible CSC provider,
  OAuth credentials, and a correctly configured instance-wide signing transport.
</Callout>
...
### Provider and transport setup

Configure a compatible CSC provider and set the required transport environment variables. VilnoEDO does not check a Documenso license flag before starting the CSC transport.
